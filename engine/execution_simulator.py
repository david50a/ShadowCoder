...
class ExecutionSimulator:
    def simulate(self, vuln, payloads: list[dict], source_code: str, tree: Optional[ast.AST] = None) -> dict:
        """
        ...
        if tree:
            try:
                ...
                import logging
                logging.getLogger("shadowcoder.simulator").error(f"AST visit failed: {e}")
        ...
        # ── Artifact Harvesting ───────────────────────────────────────────────
        harvested_artifacts = []
        if vuln.vuln_type in ("Path Traversal", "Hardcoded password", "Hardcoded API key", "Hardcoded AWS credential"):
            if exploitable or "Hardcoded" in vuln.vuln_type:
                artifact_type = "credential" if "Hardcoded" in vuln.vuln_type else "config_file"
                harvested_artifacts.append({
                    "type": artifact_type,
                    "vuln_id": vuln.vuln_id,
                    "description": f"Leaked {vuln.vuln_type} content",
                    "payload_used": payloads[0]["raw"] if payloads else "Static discovery"
                })
        ...
        if harvested_artifacts:
            exec_path.append(f"→ Harvested {len(harvested_artifacts)} attacker artifacts")
